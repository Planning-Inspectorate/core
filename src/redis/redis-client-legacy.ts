...
/**
 * A Redis Client using username and password
 * from https://github.com/Planning-Inspectorate/template-service/blob/a16e161867dbc1daea3063bb799dc2cf0ead74eb/packages/lib/redis/redis-client.ts
 */
export class LegacyRedisClient {
	...
	constructor(connString: string, logger: Logger, prefix?: string) {
		...
		const onError = (err: Error) => logger.error({ err }, `Could not establish a connection with redis server`);
