...
import { mockLogger } from '../testing/index.ts';
...
describe('auth guard', () => {
	describe('buildAssertGroupAccess', () => {
		const mockService = {
			get unauthorizedView() {
				return '403';
			}
		};
		it('should return 403 if no account', () => {
			const logger = mockLogger();
			const handler = buildAssertGroupAccess(logger, mockService);
		...
		it(`should return 403 if no groups don't match`, () => {
			const logger = mockLogger();
			const handler = buildAssertGroupAccess(logger, mockService, 'group-a');
